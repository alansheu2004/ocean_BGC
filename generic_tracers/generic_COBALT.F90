...
! Namelist Options
  ...
  type mixotroph
     ...
     real, ALLOCATABLE, dimension(:,:,:)  :: &
          ...
          jingest_n_bact, &
          jingest_n_sm, &
          psi_n, &         ! biomass contributing to autotrophy
          psi             ! mixotrophy preference parameter
...
!    call g_tracer_add_param('o2_2_nfix', cobalt%o2_2_nfix, (118.0+3.0/(5.0+3.0)*(150.0-118.0))/16.0) ! mol O2 mol N-1
    ...
    call g_tracer_add_param('smz_ipa_mx',zoo(1)%ipa_mx,1.0)         ! dimensionless
    ...
    call g_tracer_add_param('mdz_ipa_mx',zoo(2)%ipa_mx,0.0)         ! dimensionless
    ...
    call g_tracer_add_param('lgz_ipa_mx',zoo(3)%ipa_mx, 0.0)       ! dimensionless
...
!            implementation='duce',   &
    ...
    call g_tracer_add(tracer_list,package_name,&
         name       = 'psi_nmx',            &
         longname   = 'Mixotroph Autotrophic Nitrogen', &
         units      = 'mol/kg',         &
         prog       = .true.            )
...
!
    ...
    call g_tracer_get_values(tracer_list,'psi_nmx' ,'field',mixo(1)%psi_n(:,:,:),isd,jsd,ntau=tau,positive=.true.)
...
!
    !
    ! 3.1 Plankton foodweb dynamics: consumption by zooplankton, mixotrophs, and higher predators
    !
    ...
    ! 3 Small Phytoplankton
    ! 4 Mixotrophs
    ! 5 Bacteria
    ! 6 Small Zooplankton
    ! 7 Medium Zooplankton
    ! 8 Large Zooplankton
    ! 9 Detritus
    ! 
    ...
    ! Mixotrophs
    m = NUM_ZOO + 1
     ipa_matrix(m,1) = mixo(1)%ipa_diaz
     ipa_matrix(m,2) = mixo(1)%ipa_lgp
     ipa_matrix(m,3) = mixo(1)%ipa_smp
     ipa_matrix(m,4) = mixo(1)%ipa_mx
     ipa_matrix(m,5) = mixo(1)%ipa_bact
     ipa_matrix(m,6) = mixo(1)%ipa_smz
     ipa_matrix(m,7) = mixo(1)%ipa_mdz
     ipa_matrix(m,8) = mixo(1)%ipa_lgz
     ipa_matrix(m,9) = mixo(1)%ipa_det
     tot_prey(m) = 0.0
     do n = 1,NUM_PREY !{
          pa_matrix(m,n) = 0.0
          ingest_matrix(m,n) = 0.0
    ...
    prey_p2n_vec(4) = mixo(1)%p_2_n_static
    ...
    do k = 1, nk ; do j = jsc, jec ; do i = isc, iec; !{
       ...
       !
       ! 3.1.1: Calculate zooplankton & Mixotroph ingestion fluxes
       !
       ...
       ! Mixotrophs
       mixo(1)%temp_lim(i,j,k) = exp(mixo(1)%ktemp*Temp(i,j,k))
       mixo(1)%o2lim(i,j,k) = max((cobalt%f_o2(i,j,k) - cobalt%o2_min),0.0)/ & 
                         (cobalt%k_o2 + max(cobalt%f_o2(i,j,k)-cobalt%o2_min,0.0))
       ...
       prey_vec(4) = max(mixo(1)%f_n(i,j,k) - cobalt%refuge_conc,0.0)
       ...
       prey_fe2n_vec(4) = mixo(1)%q_fe_2_n(i,j,k)
       ...
       !
       ! Small zooplankton (m = 1) consuming small phytoplankton (3),
       ! mixotrophs (4) and bacteria (5).  sw_fac_denom is the denominator of the abundance-
       ...
       !
       ! Mixotrophs (m = 4) consuming small phytoplankton (3)
       ! and bacteria (5) 
       !
       ...
       pa_matrix(m,3) = ipa_matrix(m,3)* &
                        ( (ipa_matrix(m,3)*prey_vec(3))**mixo(1)%nswitch / &
                          (sw_fac_denom+epsln) )**(1.0/mixo(1)%mswitch)
       pa_matrix(m,5) = ipa_matrix(m,5)* & 
                        ( (ipa_matrix(m,5)*prey_vec(5))**mixo(1)%nswitch / &
                          (sw_fac_denom+epsln) )**(1.0/mixo(1)%mswitch)
       tot_prey(m) = pa_matrix(m,3)*prey_vec(3) + pa_matrix(m,5)*prey_vec(5)
       ingest_matrix(m,3) = mixo(1)%eta * (1-mixo(1)%psi(i,j,k)) * mixo(1)%temp_lim(i,j,k)*mixo(1)%o2lim(i,j,k)*mixo(1)%imax* &
                     pa_matrix(m,3)*prey_vec(3)*mixo(1)%f_n(i,j,k)/(mixo(1)%ki+tot_prey(m))
       ingest_matrix(m,5) = mixo(1)%eta * (1-mixo(1)%psi(i,j,k)) * mixo(1)%temp_lim(i,j,k)*mixo(1)%o2lim(i,j,k)*mixo(1)%imax* &
       ...
       !
       ! Calculate phytoplankton losses to mixotrophs & zooplankton
       !
       ...
       !
       ! Calculate mixotroph losses to zooplankton
       !

       mixo(1)%jzloss_n(i,j,k) = 0.0

       do m = 1,NUM_ZOO !{
          mixo(1)%jzloss_n(i,j,k) = mixo(1)%jzloss_n(i,j,k) + ingest_matrix(m,4)
       enddo !} m

       mixo(1)%jzloss_p(i,j,k)    = mixo(1)%jzloss_n(i,j,k)*prey_p2n_vec(4)
       mixo(1)%jzloss_fe(i,j,k)   = mixo(1)%jzloss_n(i,j,k)*prey_fe2n_vec(4) 

       !
       ! losses of bacteria to mixotrophs & zooplankton 
       !
       ...
       do n = 1,NUM_ZOO !{
          ...
          do m = 1,NUM_ZOO !{
             zoo(n)%jzloss_n(i,j,k) = zoo(n)%jzloss_n(i,j,k) + ingest_matrix(m,NUM_PHYTO+2+n)
          enddo !} m
          zoo(n)%jzloss_p(i,j,k) = zoo(n)%jzloss_n(i,j,k)*prey_p2n_vec(NUM_PHYTO+2+n)
       enddo !} n
       ...
       do n = 1,NUM_ZOO !{
         zoo(n)%jhploss_n(i,j,k) = hp_ingest_vec(NUM_PHYTO+2+n)
         zoo(n)%jhploss_p(i,j,k) = zoo(n)%jhploss_n(i,j,k)*prey_p2n_vec(NUM_PHYTO+2+n)
       enddo !} n
...
!
    ...
    do k = 1, nk ; do j = jsc, jec ; do i = isc, iec   !{
        ...
        cobalt%jprod_cadet_calc(i,j,k) = (zoo(1)%jzloss_n(i,j,k)*zoo(2)%phi_det + & 
                       phyto(SMALL)%jzloss_n(i,j,k)*zoo(1)%phi_det + mixo(1)%jzloss_n(i,j,k)*zoo(1)%phi_det + phyto(LARGE)%jzloss_n(i,j,k)*zoo(3)%phi_det + &  
                       phyto(SMALL)%jaggloss_n(i,j,k) + mixo(1)%jaggloss_n(i,j,k) + phyto(LARGE)%jaggloss_n(i,j,k))*cobalt%ca_2_n_calc* &
...
!
    ...
    call g_tracer_get_pointer(tracer_list,'psi_nmx'   ,'field',cobalt%p_psi_nmx   )
...
!
    ...
    cobalt%tot_layer_int_c(:,:,:) = (cobalt%p_dic(:,:,:,tau) + cobalt%doc_background + cobalt%p_cadet_arag(:,:,:,tau) +&
         ...
         cobalt%p_nsm(:,:,:,tau) + cobalt%p_nmx(:,:,:,tau) + cobalt%p_nbact(:,:,:,tau) + &
    ...
    cobalt%tot_layer_int_fe(:,:,:) = (cobalt%p_fed(:,:,:,tau) + cobalt%p_fedi(:,:,:,tau) + &
         cobalt%p_felg(:,:,:,tau) + cobalt%p_fesm(:,:,:,tau) + cobalt%p_femx(:,:,:,tau) + & 
    ...
    cobalt%tot_layer_int_p(:,:,:) = (cobalt%p_po4(:,:,:,tau) + &
         ...
         cobalt%p_nmx(:,:,:,tau)*mixo(1)%p_2_n_static + &
...
! CAS: spreadsheet indicates no background, should we remove it?
   ...
   cobalt%tot_layer_int_poc(:,:,:) = (cobalt%p_ndi(:,:,:,tau) + cobalt%p_nlg(:,:,:,tau) + cobalt%p_nsm(:,:,:,tau) + cobalt%p_nmx(:,:,:,tau) + &
    ...
    do j = jsc, jec ; do i = isc, iec ; do k = 1, nk  !{
          ...
          cobalt%wc_vert_int_juptake_nh4(i,j) = cobalt%wc_vert_int_juptake_nh4(i,j) +                     &
             (phyto(1)%juptake_nh4(i,j,k)+phyto(2)%juptake_nh4(i,j,k)+phyto(3)%juptake_nh4(i,j,k)+mixo(1)%juptake_nh4(i,j,k))* & 
    ...
    if (mixo(1)%id_def_fe .gt. 0)          &
      ...
      if (mixo(1)%id_jprod_srdon .gt. 0)          &
       ...
       if (mixo(1)%id_jingest_n_bact .gt. 0)          &
            ...
            model_time, rmask = grid_tmask,&
            is_in=isc, js_in=jsc, ks_in=1,ie_in=iec, je_in=jec, ke_in=nk)
       if (mixo(1)%id_psi .gt. 0)          &
            used = g_send_data(mixo(1)%id_psi, mixo(1)%psi,           &
            model_time, rmask = grid_tmask,&
            is_in=isc, js_in=jsc, ks_in=1,ie_in=iec, je_in=jec, ke_in=nk)
       if (mixo(1)%id_psi_n .gt. 0)          &
            used = g_send_data(mixo(1)%id_psi_n, mixo(1)%psi_n,           &
            model_time, rmask = grid_tmask,&
            is_in=isc, js_in=jsc, ks_in=1,ie_in=iec, je_in=jec, ke_in=nk)
    !--------------------------------------------------------------------------------------
...
!
  ...
  subroutine user_allocate_arrays
    ...
    allocate(mixo(1)%psi(isd:ied,jsd:jed,nk))     ; mixo(1)%psi      = 0.0
    allocate(mixo(1)%psi_n(isd:ied,jsd:jed,nk))     ; mixo(1)%psi_n      = 0.0
...
!==============================================================================================================
  ...
  subroutine user_deallocate_arrays
    ...
    deallocate(mixo(1)%psi)
    deallocate(mixo(1)%psi_n)
